...
- `thekids_table()` requires the font to be installed at a system level,
  else a default font family will be applied (sans).
- `theme_thekids()` can either call the font from the system (after
  registration), else a fall-back to install the font (using a Google
  API) will be called.
...
2.  Navigate to the “Create New Project” addin, or run
    `thekidsbiostats::create_project()`.
    ...
    - Branded Quarto templates can be created within the “Create
      ...
      by running `thekidsbiostats::create_template()`.
