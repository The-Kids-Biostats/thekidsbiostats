...
We have two functions which can help with this:

+ `round_vec` -- preserves trailing zeroes for vectors.
+ `round_df` -- consistently rounds every numeric column in a dataframe or tibble.
...
In comparison, `fct_case_when()` orders the factor levels based on the order of their appearance in the argument, to return an identical result to above:
